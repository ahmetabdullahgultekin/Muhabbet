 ...
 *
 * Two things had to change together, which is why both are asserted here. A `try/catch` around the
 * loop body would not have been enough — Spring Data's write methods join the surrounding
 * transaction and mark it rollback-only, so the catch would swallow an exception that had already
 * doomed the commit and the run would die at commit time reporting success. And the method-level
 * `@Transactional` had to go, because `TransactionRunner` uses REQUIRED propagation: with an outer
 * transaction still open, per-message boundaries would simply join it and the isolation would be
 * imaginary.
 */
