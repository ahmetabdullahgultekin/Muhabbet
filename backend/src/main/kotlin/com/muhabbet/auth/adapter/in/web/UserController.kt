...
class UserController(
     ...
     * target chose for everyone else, someone they blocked sees no presence, no last seen and no
     * about — a blocked harasser watching a green dot to learn when their target is awake is the
     * concrete harm one way. And someone who pressed Block sees the same nothing back: this asked
     * only whether the *target* had blocked the caller, so the person who did the blocking went on
     * watching their blocked contact's online state and last-seen stamp from the profile screen. A
     * block is a request to be done with someone, not a request to be less visible to them.
     *
     * The direction is no longer this function's to get wrong: `BlockDirectoryPort` offers
     * `blockExistsBetween` and nothing else, and the adapter is the one place the two `isBlocked`
     * questions still exist. A caller that cannot name a direction cannot pick the wrong one.
     *
     * Name and avatar still show either way, because a chat the two shared before the block would
     * otherwise become an anonymous row.
     */
